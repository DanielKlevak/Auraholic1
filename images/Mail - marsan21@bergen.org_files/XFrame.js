...
(function () {
    ...
    // To prevent token theft in case xframe is used not for getting an access token, 
    // but an id token or an auth code. In this case, the token is returned in the 
    // url query part, and can be exposed in the referer header.
    if (location.search)
        location.search = '';
    ...
    addListener('message', function (e) {
        ...
        function validateUcwaAuthURL(ucwa_oauth_uri)
        {
            //Validate auth URL to prevent XSS attack
            var validScheme='https';
            var ucwaUrl = parseUrl(ucwa_oauth_uri);
            if(!ucwaUrl || !ucwaUrl.scheme || validScheme!==ucwaUrl.scheme.toLowerCase() || !ucwaUrl.domain)
                throw new Error("UCWA Auth URL is invalid.");
        } 

        function sendRequestViaAuthXframe(request, ucwa_oauth_uri) {
            validateUcwaAuthURL(ucwa_oauth_uri);
            var iframe;
